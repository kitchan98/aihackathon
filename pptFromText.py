...
for slide_data in slides_data:
    ...
    slide_layout = layout_mappings.get(layout_index, prs.slide_layouts[0])  # Default to Title Slide layout if invalid layout is specified
    ...
    if 'title' in slide_info:
        ...
        if layout_index == '9':  # For slides with Picture with Caption layout
            ...
            title_shape.text_frame.paragraphs[0].font.size = Inches(.25)

    # Set content
    if content_lines:
        content_shape = slide.placeholders[1]
        content_frame = content_shape.text_frame
        for line in content_lines[1:]:  # Exclude the first line if it's not a part of the title
    ...
    if layout_index == '9':  # Picture with Caption layout
        ...
        caption_text = slide_info.get('content', '')  # Get the text from the property file
        if caption_text:
            caption_shape = slide.placeholders[2]  # Assuming the text placeholder is the third placeholder
